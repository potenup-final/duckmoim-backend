 ...
 *
 * <p><b>서비스로 만들지 않고 SQL 로 넣는 이유가 둘이다.</b> 첫째로 상태를 한 줄로 정할 수 있다 — DELETED · BLINDED 로 가는 전이가 이제
 * 있지만(CM-10 · AD-07), 그것을 지나면 검사마다 남의 요구사항이 선행으로 붙는다. 둘째로 대댓글을 부모로 지정하는 검증에 필요한 「이미 대댓글인 댓글」을 도메인
 * 팩터리로는 저장 없이 만들 수 없다.
 *
