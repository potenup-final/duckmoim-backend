...
/**
 * 메시지를 가리고 감사 로그를 남긴다 (AD-09).
 *
 * <p><b>트랜잭션 경계가 이 클래스다.</b> 흐름은 {@link AdminMessageBlindService} 가 진다 — 커밋된 뒤에 방에 알려야 해서 빈을 나눴다
 * ({@code ChatMessageDeleteWriter} 와 같은 배치).
 *
 * <p><b>상태 변경과 감사 로그가 한 트랜잭션이다.</b> 감사 로그가 실패해 롤백되면 가린 것도 되돌아가는데, 그때 이미 방에 알렸다면 화면만 가려진 상태가 남는다 —
 * 알림을 이 트랜잭션 밖으로 뺀 이유다.
 *
...
public class AdminMessageBlindWriter {
