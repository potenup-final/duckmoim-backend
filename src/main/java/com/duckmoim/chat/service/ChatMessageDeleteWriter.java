...
/**
 * 메시지 삭제를 판정하고 저장한다 (CH-12).
 *
 * <p><b>트랜잭션 경계가 이 클래스다.</b> 흐름은 {@link ChatMessageDeleteService} 가 진다 — 이 트랜잭션이 커밋된 뒤에 방에 알려야 해서
 * 빈을 나눴다 ({@code ChatMessageWriter} 와 같은 배치). 같은 클래스 안에서 자기를 부르면 프록시를 지나지 않아 {@code @Transactional}
 * 이 걸리지 않는다.
 *
...
public class ChatMessageDeleteWriter {
