 ...
 *
 * <p><b>다만 그 안에서 소켓 I/O 를 기다릴 수 있다</b> (PR #142 리뷰). 끊기는 {@code SseEmitter#complete} 까지 내려가고, 그
 * 메서드는 {@code ResponseBodyEmitter.writeLock} 을 잡는다 ({@code spring-webmvc 6.2.19} 바이트코드로 확인 — {@code
 * send} · {@code complete} 가 같은 락이다). 하트비트 스레드가 <b>정체된 그 연결에 쓰다 막혀 있으면</b> 닫기가 그만큼 기다리고, 그 시간 내내 이
 * 트랜잭션이 DB 커넥션을 쥔다.
 *
 * <pre>
 * 하트비트 스레드  emitter.send(주석)  ──▶ 송신 버퍼가 찬 모바일. 수십 초 반환 안 함
 *                                         writeLock 점유
 * 퇴장 (이 트랜잭션)  complete() ─ writeLock 대기 ─▶ DB 커넥션을 쥔 채로 함께 기다린다
 * </pre>
 *
 * <p><b>지금 고치지 않았다.</b> 대상이 <b>나가는 본인의 연결 하나</b>라, 자기 소켓이 막힌 상태에서 퇴장 요청을 보내야 겹친다 — 그 요청은 같은 소켓이 아니라
 * 새 연결로 오므로 가능하지만 드물다. 고치려면 끊기를 커밋 뒤로 빼야 하는데, 그러면 바로 아래 문단의 판단(커밋과 끊기 사이 창)을 뒤집는 거래가 된다. <b>둘 중 하나를
 * 고를 문제이고 지금은 아래쪽을 택한 상태다.</b>
 *
