...
public interface UserRepository extends JpaRepository<User, Long> {
   ...
   *
   * <p><b>상태로 걸러내지 않는다.</b> 유니크 제약이 상태를 보지 않으므로 사전 조회도 보지 않아야 판정이 갈리지 않는다. 여기서만 걸러내면 「사용 가능」이라고 답한
   * 닉네임이 저장에서 409 가 되어, <b>사용자가 원인을 알 수 없는 실패</b>를 본다.
   *
   * <p><b>탈퇴 회원의 닉네임은 애초에 세어지지 않는다</b> — AU-11 이 탈퇴 시 닉네임을 비우므로 값이 {@code NULL} 이고, 유니크 제약도 이 조회도
   * NULL 을 세지 않는다. <b>탈퇴하면 그 이름이 풀려 남이 다시 쓸 수 있다.</b>
   *
   * <p>다만 AU-11 이전에 만들어진 행은 닉네임을 그대로 갖고 있다 ({@code V11} 시드의 탈퇴 회원이 그렇다). 그 행이 계속 세어지는 것이 맞다 — 유니크
   * 제약이 세는 것과 같아야 한다.
   */
